...
class KRSController extends Controller {
	...
	public function index() {
		...
		return view($folder.'.index', 
			compact('title','redirect','folder','th_akademik_id',
			'list_thakademik','prodi_id','list_prodi','kelas_id','list_kelas')
		);
	}
	...
	public function getData(Request $request){
		...
		->with('th_akademik','prodi','kelas','mahasiswa')->get();
		...
		->addColumn('action',function($row){
			...
			$btn = $btn.'<a onclick="deleteForm('.$row->id.')" class="btn btn-danger btn-xs btn-rounded tooltip-danger" 
	...
	public function getMhs(Request $request){
		...
		->with('prodi','kelas','kelompok','jk','status')->first();
	...
	public function getMhs_old(Request $request){
		$th_akademik = ThAkademik::Aktif()->first();
	...
	public function getDataMK(Request $request){
		...
		->join('trans_kurikulum_matakuliah','trans_kurikulum_matakuliah.id','=',
		...
		->addColumn('cek_list',function($row) use ($th_akademik_id,$nim){
			...
			if($krs){
				...
				->where('jadwal_kuliah_id',$row->id)
				->where('nim',$nim)
				->whereNotNull('nilai_huruf')->first();

				if($krs_nilai){
					return '<i class="fa fa-check text-success"></i>';
				}else{
					return '<input type="checkbox" name="cek_list[]" id="cek_list_'.$row->id.'" value="'.$row->id.'" 
					onClick="cekList(\''.$row->id.'\',\''.$row->kurikulum_matakuliah->matakuliah->sks.'\')" checked >';
				}
			}else{
				return '<input type="checkbox" name="cek_list[]" id="cek_list_'.$row->id.'" value="'.$row->id.'" 
				onClick="cekList(\''.$row->id.'\',\''.$row->kurikulum_matakuliah->matakuliah->sks.'\')"  >';
			}
			...
			//   }else{
			//     return '<a onclick="deleteDetail('.$data->id.')" 
			// class="btn btn-danger btn-xs btn-rounded tooltip-danger" data-toggle="tooltip" 
			//data-placement="top" data-original-title="Delete"><i class="fa fa-times"></i></a>';
			//   }
			//   // return '<input type="checkbox" name="cek_list[]" id="cek_list" value="'.$row->id.'" 
			//onClick="cekList(\''.$row->id.'\')" checked >';
			// }else{
			//   return '<input type="checkbox" name="cek_list[]" id="cek_list" value="'.$row->id.'" 
			//onClick="cekList(\''.$row->id.'\')"  >';
			// }
		})
		...
		->addColumn('nama_mk',function($row){
			...
			// return  @$row->kurikulum_matakuliah->matakuliah->nama.' - '.
			// @$row->kurikulum_matakuliah->matakuliah->id;
		})
	...
	public function create(){
		...
		// $list_mhs = Mahasiswa::where('status_id',18)->orderBy('nim','DESC')->get();
		$nim = null;
		return view($folder.'.create', compact('title','redirect','folder','th_akademik','list_mhs','nim')
		);
	}
