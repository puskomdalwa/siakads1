...
<form class="form-horizontal form-borderd" name="form-input" id="form-input" action="{{route($redirect.'.cetak')}}" method="post" target="_blank">
	{{ csrf_field() }}

	<div class="panel-body no-padding-hr">
		<div class="form-group{{ $errors->has('th_akademik_id') ? ' has-error' : '' }} no-margin-hr panel-padding-h no-padding-t no-border-t">
		<div class="row">
			<label class="col-sm-2 control-label text-danger">Tahun Akademik: {{route($redirect.'.cetak')}}</label>
			<div class="col-sm-2">
			...
			<div class="col-sm-3">
				<select class="form-control" name="kelas_id" id="kelas_id">
					@if(empty($kelas_id)) <option value=""> -Semua- </strong> @endif
