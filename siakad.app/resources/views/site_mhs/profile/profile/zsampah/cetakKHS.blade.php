...
<div style="font-size:12px;">
	...
	<table>
		<thead>
			<tr>
				<td width="13%">NIM yyyyy</td><td>: {{$krs->nim}}</td>
				<td width="13%">Tahun Akademik</td>
				<td>: {{ substr($krs->th_akademik->kode,0,4) }} / {{ $th_akademik->semester }} </td>
			</tr>

			<tr>
				<td>Nama Mahasiswa</td><td>: {{$krs->mahasiswa->nama}}</td>
				<td>Kelas / Semester</td><td>: {{$krs->kelas->nama}} / {{$krs->smt}}</td>
			</tr>

			<tr>
				<td>Program Studi</td><td>: {{$krs->prodi->nama}}</td>
				<td>IP smt Lalu</td><td>: {{getIP($krs->nim,$krs->th_akademik_id,$krs->smt-1)}}</td>
			</tr>
		</thead>
	</table>
	...
	<table class="data">
		...
		<tfoot>
			...
			<tr>
				<td class="text-center" colspan="3" style="background:#d6d6d6;">IP</td>
