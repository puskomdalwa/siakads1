...
<div style="font-size:12px;margin-left:20px;margin-right:10px;">
    <table class="judul">
        <tr>
            ...
            <th class="text-center" width="85%" style="vertical-align:top">
                ...
                <span style="font-size:18px;">TAHUN AKADEMIK {{ $th_akademik->nama }}
                    {{ $th_akademik->semester }}</span><br />
                <span class="alamat">Alamat : {{ $pt->alamat }} - {{ $pt->kota->name }}. Telp:{{ $pt->telp }}
                    Email : {{ $pt->email }}</span>
            </th>
        </tr>
    </table>
    ...
    <u>
        <h2 class="text-center" style="font-size:16px;margin:0px;">SURAT KETERANGAN AKTIF KULIAH</h2>
    </u>
    ...
    <table class="table table-striped table-hover" style="font-size:12px;">
        <tbody>
            <tr>
                ...
                <td>: <strong>{{ strtoupper($mhs_aktif->nama) }}</strong> </td>
            </tr>
            <tr>
                <td>NIM</td>
                <td>: {{ strtoupper($mhs_aktif->nim) }} </td>
            </tr>
            <tr>
                <td>Jenis Kelamin</td>
                <td>: {{ strtoupper(@$mhs_aktif->jk->nama) }} </td>
            </tr>
    ...
    yang tersebut namannya di atas sedang Aktif Kuliah pada Program Studi
    {{ strtoupper($mhs_aktif->prodi->nama) }} semester
    {{ @getSemesterMahasiswa($mhs_aktif->th_akademik->kode, $mhs_aktif->nim) }}
    ({{ @terbilang(getSemesterMahasiswa($mhs_aktif->th_akademik->kode, $mhs_aktif->nim), 1) }}).
    <br /><br /><br /><br />
