<div class="panel widget-messages-alt panel-danger panel-dark">
    <div class="panel-heading">
        <span class="panel-title"><i class="panel-title-icon fa fa-laptop"></i>{{ $title }} Banin</span>
    </div> <!-- / .panel-heading -->
    <div class="panel-body no-padding-hr">
        <div class="table-responsive">
            <table id="table-banin" class="table table-hover table-bordered">
                <div id="table-loader-banin" class="table-loader"></div>
                <thead>
                    <tr>
                ...
                <tbody>
                    @for ($i = 1; $i <= $jumlahDosen; $i++)
                        @php
                            $kompreDosen = \App\KompreDosen::where('penguji', $i)->where('jenis_kelamin', 'L')->first();
                            $cekKompre = false;
                            if ($kompreDosen) {
...
@push('scripts')
    <script>
        let tableBanin = $('#table-banin').DataTable({
            "order": [
                [0, "asc"]
            ]
        });
