...
<div class="table-responsive">
	<table id="keuanganTable" class="table table-bordered table-condensed ">
		<thead>
			<tr>
			...
			<th class="text-center col-md-1">Hutang</th>
			</tr>
		</thead>
		
		<tfoot>
			<tr>
			<th colspan="3" style="text-align:center">Jumlah Tagihan / Bayar / Hutang</th>
...
var dataTableKeuangan = $("#keuanganTable").DataTable({
	...
	columns: [
		...
		{ data: 'dw_tanggal', 	name: 'dw_tanggal'},
		{ data: 'dw_bayar',		name: 'dw_bayar','class':'text-right'},
		{ data: 'idn_tanggal', 	name: 'idn_tanggal'},
		{ data: 'idn_bayar', 	name: 'idn_bayar','class':'text-right'},
	...
	"footerCallback": function ( row, data, start, end, display ) {
		...
		{ style: 'currency', currency: 'IDR' }).format(jml_tagihan));
		// Total over all pages
		...
		{ style: 'currency', currency: 'IDR' }).format(dw_jmlbayar));
		// Total over all pages   
		...
		{ style: 'currency', currency: 'IDR' }).format(idn_jmlbayar));
		
		$( api.column(8).footer() ).html(new Intl.NumberFormat('in-RP', 
		{ style: 'currency', currency: 'IDR' }).format(hutang));
