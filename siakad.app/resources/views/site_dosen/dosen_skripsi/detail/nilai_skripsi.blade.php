<div class="container" id="nilai_skripsi">
    <div class="card">
        ...
        <div id="update_nilai_skripsi">
            ...
            <hr>
            <form id="form_nilai_skripsi"
                action="{{ route('dosen_skripsi.simpanNilaiSkripsi', ['id' => $pengajuan->id]) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="form-group">
                    <label for="nilai_skripsi_angka">Nilai Angka</label>
                    <input type="number" class="form-control" oninput="setNilaiHuruf(this)" name="nilai_angka"
                        value="{{ $pengajuan->nilai_angka }}" id="nilai_skripsi_angka" step="any">
                ...
                @if ($ujianSkripsi->status == 'lolos')
                    <button type="submit" class="btn btn-primary" id="form_nilai_skripsi_submit"><i
                            class="fa fa-plus-circle" aria-hidden="true"></i> Simpan</button>
                    <button type="button" class="btn btn-danger" id="kosongkan_nilai_skripsi"
...
@push('scripts')
    <script>
        ...
        $('#form_nilai_skripsi').submit(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('dosen_skripsi.simpanNilaiSkripsi', ['id' => $pengajuan->id]) }}",
        ...
        function kosongkanNilaiSkripsi() {
            swal({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{ url($redirect) }}" + '/detail/{{ $pengajuan->id }}/kosongkanNilaiSkripsi',
                        type: "POST",
                        data: {
                            '_method': 'PUT',
                            '_token': csrfToken
