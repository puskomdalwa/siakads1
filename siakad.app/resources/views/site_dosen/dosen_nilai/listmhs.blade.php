...
<div class="panel panel-success panel-dark">
    <div class="panel-heading">
        <span class="panel-title">
            @if (($tgl >= $form->tgl_mulai && $tgl <= $form->tgl_selesai) || \Auth::user()->id == 69)
                <h4><b>
...
<div class="all-open">
    <button type="button" class="btn btn-success" id="step_5" style="width:100%" onclick="allOpen()">Buka semua detail
        nilai
...
@push('scripts')
            ...
            <form onsubmit="saveNilai(event, @{{ id }})" id="form_save_nilai_@{{ id }}">
                ...
                <table class="table table-condensed table-bordered table-hover table-striped" id="isi_nilai-@{{id}}">
                    ...
                    <tfoot>
                        ...
                        @else
                            @if ($tgl > $form->tgl_selesai)
                                <th colspan="9" style="text-align:center"> Mohon Maaf, Pengisian Nilai Sudah Ditutup !!!</th>
                            @else
                                <th colspan="9" style="text-align:center"> Mohon Maaf, Pengisian Nilai Belum Ditutup !!!</th>
                            @endif
                        @endif
                        </tr>
                    </tfoot>
    ...
    <script type="text/javascript">
                ...
                "footerCallback": function(row, data, start, end, display) {
                    @if (($tgl >= $form->tgl_mulai && $tgl <= $form->tgl_selesai) || \Auth::user()->id == 69)
                        ...
                        var api = this.api(),
                            data;
                        var element =
                            `<button type="submit" id="simpan_nilai-${idKrsDetail}" class="btn btn-success" style="width:100%">Simpan Per Mahasiswa</button>`;
        ...
        function saveOperation(idKrsDetail) {
            ...
            var url = "{{ route('dosen_nilai.saveNilai', ['id' => $id]) }}";
            $.ajax({
                url: url,
                type: "POST",
        ...
        function initDataTable() {
            var url = "{{ route('dosen_nilai.getDataNilai', ['id' => $data->id]) }}";
            return $("#serversideTable").DataTable({
                ...
                lengthMenu: [100, 30, 40, 50, 70, 200],
                // paging: false,
                columns: [{
